...
PROMPT_GEOMETRY_GIT_TIME=${PROMPT_GEOMETRY_GIT_TIME:-false}
...
prompt_geometry_git_status() {
  if test -z "$(git status --porcelain --ignore-submodules -uno)"; then
...
prompt_geometry_render() {
 ...
 #  PROMPT="
 # %${#PROMPT_SYMBOL}{%(?.$GEOMETRY_PROMPT.$GEOMETRY_EXIT_VALUE)%} %F{$GEOMETRY_COLOR_DIR}%1~%f "
  PROMPT="
 %{$fg_bold[$GEOMETRY_COLOR_DIR]%}%1~%f%b "
